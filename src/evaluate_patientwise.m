...
%EVALUATE_PATIENTWISE Patient-grouped CV with train-only NCA and scaling.
...
for k=1:cfg.numFolds
    ...
    % Train-only scaling for the classifier.
    ...
    rows=[rows; table(k,sum(tr),sum(te),numel(unique(subjectID(tr))), ...
        numel(unique(subjectID(te))),m.accuracy,m.balancedAccuracy,m.macroF1, ...
        'VariableNames',{'Fold','NTrain','NTest','TrainSubjects','TestSubjects', ...
        'Accuracy','BalancedAccuracy','MacroF1'})]; %#ok<AGROW>
    save(fullfile(cfg.resultsDir,sprintf('%s_fold%02d_nca.mat',pipelineName,k)), ...
        'localIdx','weights','mu','sd');
end
...
summary.perClass=allM.perClass;
summary.confusion=allM.confusion;
summary.classes=allM.classes;
